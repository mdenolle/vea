...
function  [R,T,Z]=irf2dc(M,FT,bigC,ZZ,ZR,RZ,RR,TT,HS,az,Sh,dr)
...
for ifreq=2:length(bigC)
    ...
    if Nmode==1;
        imode=1;
        ux0(imode)=C(1).ux(end);uz0(imode)=C(1).uz(end);uy0(imode)=C(1).uy(end);
        ...
        for imode=1:Nmode
            uxi(imode) = interp1(C(1).zz(ib),C(1).ux(ib,imode),HS,'linear');
            uyi(imode) = interp1(C(1).zz(ib),C(1).uy(ib,imode),HS,'linear');
            uzi(imode) = interp1(C(1).zz(ib),C(1).uz(ib,imode),HS,'linear');
            duxi(imode) = -interp1(C(1).zz(ib),C(1).Dux(ib,imode),HS,'linear');
            duyi(imode) = -interp1(C(1).zz(ib),C(1).Duy(ib,imode),HS,'linear');
            duzi(imode) = -interp1(C(1).zz(ib),C(1).Duz(ib,imode),HS,'linear');
            ux0(imode)=C(1).ux(end,imode);uz0(imode)=C(1).uz(end,imode);
            uy0(imode)=C(1).uy(end,imode);
        end
    end
    for ir=1:length(ZZ)  
        if isempty(ZZ{ir});continue;end
        NmodeR = min(Nmode,min(size(ZZ{ir})));
        NmodeL = min(Nmode,min(size(TT{ir})));
        Th{ir}(ifreq) =0;
        Rh{ir}(ifreq) =0;
        Zh{ir}(ifreq) =0;
        ...
        for imode=1%:NmodeL
            ...
            Th{ir}(ifreq) =  Th{ir}(ifreq) +( 1i*C(1).kl(imode)*M2{ir}(2,1)*uyi(imode)/uy0(imode) ...
                + duyi(imode)/uy0(imode)*M2{ir}(2,3)   )*TT{ir}(imode,ifreq).*exp(-1i*C(1).omega*DT(imode));
        end
        for imode=1%1:NmodeR
            DT(imode)=dr{ir}/C(1).cr(imode);
            Rh{ir}(ifreq) =  Rh{ir}(ifreq)+ ((1i*C(1).kr(imode)*M2{ir}(3,1)*uzi(imode)/uz0(imode) + ...
            ...
            Zh{ir}(ifreq) = Zh{ir}(ifreq)+  ( ( 1i*C(1).kr(imode)*M2{ir}(1,1)*uxi(imode)/ux0(imode) + ...
